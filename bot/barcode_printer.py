...
import psutil
from progress import generate_barcode
import pythoncom
def kill_task():
    task_name = 'WORD.EXE'
...
async def update_document(template_path, new_file_path, name, barcode_image_path):
...
async def print_barcode_barcode(word_name, data_to_encode, name, barcode_name='barcode.png', page=1):
    try:
        new_file_path = f'{word_name}.docx'
        barcode_image_path = await asyncio.to_thread(generate_barcode, data_to_encode, barcode_name)
        saved_file_path = await update_document('barcode.docx', new_file_path, name, barcode_image_path)
